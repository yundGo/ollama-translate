const LOG_PREFIX = '[Local Translate BG]';
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'translate') {
    ...
    translateText(message.text, message.model, message.prompt, endpoint, controller.signal)
      .then((translated) => {
...
async function translateText(text, model, prompt, endpoint, signal) {
...
async function listModels(endpoint) {
  const baseUrl = (endpoint || DEFAULTS.endpoint).replace(/\/v1\/chat\/completions\/?$/, '');
